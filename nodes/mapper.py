...
class MapperNode(Node):
    ...
    def init_params(self) -> None:
        """Declares mapper parameters from 'mapping_params.yaml'."""
    ...
    def get_map_meta_data(self) -> MapMetaData:
        """Converts map size & data into MapMetaData msg type.

        Returns:
            MapMetaData: Discrete description of our map.
        """
        ...
        # meta_data.origin.position.x = 0   # not necessary -> initialized with 0
        # meta_data.origin.position.x = 0   # not necessary -> initialized with 0
    ...
    def publish_map(self, data: np.ndarray, pub: Publisher) -> None:
        msg = OccupancyGrid()
    ...
    # -------------------------------------------
    def include_obstacle_in_map(self, obstacle: Polygon, cells: np.ndarray) -> np.ndarray:
        """Includes obstacles in the uninflated map

        Args:
            obstacle (Polygon): Obstcles
            cells (np.ndarray): uninflated map from before

        Returns:
            np.ndarray: Map from before, where obstacles have been added
        """
        ...
        for index, point in enumerate(obstacle.points):
            obstacle_cells[index, 0] = int(round(point.x / self.cell_resolution))
            obstacle_cells[index, 1] = int(round(point.y / self.cell_resolution))

        img = cells.astype(dtype='uint8')   # Convert values in cells array to uint8
    ...
    def inflate_obstacles(self, cells: np.ndarray) -> np.ndarray:
        """Infaltes all obstacles in our map.

        Args:
            cells (np.ndarray): Discrete map from before.

        Returns:
            np.ndarray: Discrete map with inflated obstacles.
        """
        ...
        dilatation_size = int(np.round(INFLATION_SIZE / self.cell_resolution))
